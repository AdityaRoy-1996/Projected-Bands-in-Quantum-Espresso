...
def quantum_state(nspin, state, states):
    ...
    Returns
    -------
    Maximum number of orbitals for a given shell, as integer
...
def read_band_energies(filename):
    ...
    # band_array   =    remove_doscontiniouty(band_array = band_array)
  
    return(band_array)

def remove_doscontiniouty(band_array) :
    '''
    To remove linear interpolation at discontinious symmetry points

    Parameters
    ----------
    band_array : 2-D Numpy Array
        DESCRIPTION   :  Used inside read_band_energies()

    Returns
    -------
    Input array with np.nan at discontiniouties

    '''
    diff = np.abs(np.diff(band_array[:,0]))
...
def read_kpoints(filename) :
    ...
    Returns
    -------
    knames  :   Names of the Hogh Symmetry K Points
    '''
...
def read_bandsx_out(filename) :
    ...
    # kticks   =    get_common_from_list(kticks).sort()
